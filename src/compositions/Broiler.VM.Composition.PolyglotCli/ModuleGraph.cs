...
/// <remarks>
/// <para>
/// <b>THIS FILE IS COPIED, LINE FOR LINE, FROM
/// <c>Broiler.VM.Composition.JavaScript.Cli/ModuleGraph.cs</c>, apart from the namespace and the
/// two names the reader type changed.</b> Rules A11 and A12 leave shared code between composition
/// roots nowhere to live - a library holding it would be a project outside
/// <c>src/compositions/</c> naming a profile assembly, which A11 forbids, and a root referencing
/// another root is not the reference set A12 admits - so copying is the normal shape here and is
/// recorded rather than hidden. A resolution rule that DIFFERED between two end-user hosts of the
/// same profile would be worse than a duplicate: two hosts would disagree about what
/// <c>import "./m.mjs"</c> names, and neither would say so.
/// </para>
/// <para>
...
internal static class ModuleGraph
{
    ...
    internal static Loaded Load(string rootPath)
    {
        ...
        while (pending.Count != 0)
        {
            ...
            var file = Inputs.Read(key);

            if (file.Problem.Length != 0)
            {
                return new Loaded([], $"`{key}`: {file.Problem}");
            }
