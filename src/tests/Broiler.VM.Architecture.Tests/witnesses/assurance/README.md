...
| J8 | The record naming every alias the tree carries, with the counts that separate a live decision from one the code has outrun. Two aliases, one of each, because the document this replaced had one signature line and therefore room for one person - and because nothing in this component names anybody, so the multi-alias answer exists nowhere else. The generator and the declared shape are asserted to agree on it, which is the derivation being checked rather than restated. The manifest-header and file-header clauses keep their own files; the report and record clauses drive the real artefact with one line put into it, because a stored copy of a generated artefact goes stale at every regeneration. |
...
| J11 | A tree whose record says everything a publish needs it to say - every relevant unit carrying a decision the current code answers to - which exists nowhere in this component and has to be synthesized, or the release gate would be a function nobody has watched return an empty list. It carries `Fingerprint=TBF` and a bare alias rather than baked values, and the test drives it through the generator before asking the gate anything: a witness holding computed fingerprints would go stale the moment normalization changed. The five rejecting directions are that same tree with one thing altered, each asserted by the content of the blocker it expects. |
