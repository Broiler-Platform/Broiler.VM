...
| J2 | Each closed vocabulary, each required field, each malformed-exemption shape, a field with no `=`, a well-formed block attached to no declaration, and a `Spec=` citation naming an ADR that is not in `docs/adr/`. |
