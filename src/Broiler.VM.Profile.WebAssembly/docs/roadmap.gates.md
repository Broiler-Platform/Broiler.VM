...
or risk below said before it was replaced. **A risk row is never deleted because it was narrowed or
because its stop condition fired**: a register that removes the rows that caught something teaches
nothing, so the row stays with its outcome recorded.
...
|---|---|---|
| Dependency architecture | acyclic graph asserted against a checked-in manifest in both directions; exact profile reference set read from project text and from metadata; no edge to another profile family in either direction, inbound recording its branch; no reference to the script reader, the corpus store, or the encoder from any ADVERTISED composition root, the harness roots that legitimately contain them being named in the register; no dynamic loading, reflection invocation, IL emit, reflective member write, or module initializer; no aggregate profile-listing type; namespace-matches-assembly scan; per-clause witnesses | any forbidden project or assembly edge, an unresolvable build item cleared as a pass, an advertised closure reaching the ingestion path, a harness placed in a test project where the rules forbid it to reference a profile, undeclared dynamic loading, a registered rule with no witness |
...
| Licence and attribution | the host component's notice carrying a row for the ingested suite's attribution, landed by this profile **at the milestone that first ingests a suite file** rather than inherited from adoption or written before the material exists; modified files marked; the core's standing third-party claim confirmed scoped or amended with the release owner's co-signature | an attribution obligation discovered during a publish, a standing claim elsewhere falsified by what this component's tree contains |
...
1. **Support truth:** the support table names the implemented and minimum-accepted core contract
   ...
   primitives this profile replaces and why, **and which budget dimension changes side as a
   consequence** — declining the guarded count reader makes `DeclaredCount` this profile's own
   charge where the core's metering split calls it core-metered — because a reader of that split
   who is not told which row this profile leaves attributes a check to the core that the core is not
   performing; every unimplemented capability has a named
   deterministic failure or a named exclusion, threads, shared memory, multi-result host imports and
   the JavaScript API among them; **the amendment register's state is published row by row, with the
   deterministic failure or named exclusion each held row leaves standing and with the procedure's
   unexecutability stated**, and where the refusable retention member was not answered **the
   resulting `memory.grow` deviation is named with its deterministic behaviour**, because a held
   amendment is an unimplemented capability and this gate does not admit an unnamed one; **the
   declared default vector is published as the neighbour-facing half with its reconciliation named
   as unowned**; composition label, contract admission, and implemented feature are
...
2. **Graph and registration:** the graph is acyclic and matches its manifest; the profile reference
   ...
   reaches another profile family in either direction; no advertised composition root reaches the
   ingestion path, and the harness roots that do are named in the register and never advertised;
   registration is static and typed, with no reflection, dynamic loading, IL emit, or module
   initializer anywhere in an advertised closure.
...
| A core record this profile falsifies is left standing, because the falsification is convenient for nobody to raise. The core's metering split obliges every profile to route declared counts through the binary package; [section 7](roadmap.md#7-the-artifact-the-decoder-and-one-disagreement-with-the-core) establishes that this profile cannot, and that is a claim in the core's record rather than a defect in this one. | WA-2's exit gate names the dimension that changes side and reads the core's record against that answer, confirming the conditional reading or filing a correction with its owner and recording the row as open; WA-10 carries the naming into the support table, where a reader of the core's split meets it. **Stop: a core record this profile has falsified and not reported is a defect this component leaves behind, and it is not discharged by this component's own support table being right.** |
...
| The ingestion path — a text-format reader, a binary encoder, and a large third-party corpus — leaks into a shipped image. **It cannot be kept out by putting it in a test project: the host component's active rules forbid a test project to reference a profile assembly, so the harness is a composition root that publishes a closure of its own** *(corrected: WAC-22)*. | [Section 5](roadmap.md#5-package-boundaries-and-the-dependency-graph) settles the placement before WA-0 draws a shell; the register names which roots are advertised; architecture rules with both halves; a scan over every package and every ADVERTISED closure; and a negative control that adds the reference from the execution root and observes the failure. **Stop: an advertised closure containing the ingestion path falsifies the "no text format, no compiler" claim that is most of this profile's value as a counterweight — and a scan written over "every published closure" would fail on the harness root and be relaxed into meaninglessness, which is the second-order defect this row now names.** |
...
| This profile's declared **defaults** silently constrain a second profile composed beside it in a browser, wherever the host adopts defaults rather than stating ceilings. | WA-0 records the defaults with the cross-profile consequence stated in the decision itself; [section 17](roadmap.md#17-the-cross-profile-boundary-the-javascript-api-for-webassembly) names the reconciliation as belonging to whichever component composes both. **The risk is the defaults and only the defaults**: a maximum binds only the modules of the profile that declared it and reaches no neighbour *(corrected: WAC-01)*. **Stop: a guest-load default of zero is a composition defect, and it is caught by a two-profile catalog test rather than by a reader** — as is a guest-load default of `Unconstrained`, which the catalog refuses outright *(corrected: WAC-10)*. |
...
| The programme stalls on a precondition this component does not control. | The core-acceptance blocker is recorded with its holder and its unblock condition; the decisions that need no code are opened against WA-1. **The refusable retention amendment is the second such precondition and is recorded the same way** — holder, unblock condition, and a fallback that is WA-10's release decision rather than WA-5's, so an unexecutable amendment procedure can neither end the programme quietly nor be worked around inside the milestone it blocks *(corrected: WAC-16)*. **Stop: a milestone blocked by a named external dependency is recorded blocked with its holder and its unblock condition — lack of scheduling is not a blocker, and an unaccepted contract is.** |

Stop or re-scope a milestone when the graph is cyclic; a product closure reaches dynamic code, the
ingestion path, or another profile family; a validator cannot produce an immutable bounded
...
**No reference here resolves into any other Broiler profile family**, and no specification
