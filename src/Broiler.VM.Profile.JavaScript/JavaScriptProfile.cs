...
public static class JavaScriptProfile
{
    ...
    private static VmLimitVector Maxima()
    {
        ...
        // interpreter survives 19,288 JavaScript calls on the sixty-four-megabyte stack
        // `JsExecution.GuestStackBytes` declares - one call costing 3,479 bytes of native stack,
        ...
        // generator family and `with` all in the set. Admitting the generators took the per-call
        // cost from 3,158 bytes to 3,463 and the capacity from 21,246 calls to 19,377; admitting
        // `with` took it to 3,479 bytes and 19,288 calls, which is still more than twice this row -
        // so the ordering the row exists to guarantee still holds.
