...
|---|---|---|
| Dependency architecture | acyclic graph asserted against a checked-in manifest in both directions; exact profile reference set read from project text and from metadata; no edge to a legacy component in either direction, inbound recording its branch; no dynamic loading, reflection invocation, IL emit, reflective member write, or module initializer; no aggregate profile-listing type; namespace-matches-assembly scan; per-clause witnesses; **the composition register naming what each root contains beyond the image it demonstrates, since a root that carries the corpus writer, the fuzz mutator and the soak carries them into its own closure** *(corrected: JSC-34)* | any forbidden project or assembly edge, an unresolvable build item cleared as a pass, undeclared dynamic loading, a namespace that does not match its assembly, a registered rule with no witness, **an advertised composition whose closure carries evidence-producing code** |
...
| Format and verifier safety | five verifier outcomes each by a named case; **a corpus entry per exhaustion dimension the verifier can name, each recording the dimension and the scope the answer carried — the reader's three, structural depth, section count and declared counts; the allocator's and the work charge's two, allocated bytes and verifier work; the poll's one, the wall clock; and artifact bytes, which the core answers before the verifier is entered — because an exhaustion answer carries no diagnostic code and the registry's binding therefore reaches none of the seven** *(corrected: JSC-39, JSC-41)*; retained malformed corpus with expected-and-observed triples and successful control entries; double replay with no residue; ordering assertions — ceilings before the first byte, refusal before allocation, bound before declared-count use; capability-absent and capability-throws verification; caller-buffer mutation, disposal, and concurrent overwrite after return; bounded-read statuses mapped; corpus extended at every format-growing milestone and replayed through the nested path; guided fuzzing with minimized regressions, the guidance keyed on the answer this profile publishes rather than on an edge *(corrected: JSC-42)* | invalid input executes, a verifier throws, a late check is reported as a language fault, a declared count sizes an allocation before its bound comparison, a corpus in which nothing verifies successfully, a fuzz counterexample closed by an allow-list entry, **a ceiling breach recorded as an invalid artifact or a framing failure recorded as exhaustion** |
...
| Conformance | pinned suite revision; self-check with failing **and** passing fixtures before every shard, plus an injected-and-reverted scoring regression; per-host-mode totals; negative-metadata totals; merge configuration-failure kinds; failure manifest as a queue; ratchet not regressed; per-manifest attribution; the harness's own regression suite; **the ingestion path — the harness, its suite cache and every suite file — absent from every package and from every ADVERTISED composition's closure, the never-advertised harness root that legitimately contains it being named in the register, with a negative control that adds the reference from the execution-only root** *(corrected: JSC-40)* | a failing test reported as a pass, a mode selecting files and executing none, a green run with zero executed tests, a regression against the ratchet, a claimed manifest whose totals show it failing, an advertised closure reaching the ingestion path, a harness placed in a test project where the rules forbid it to reference a profile |
| Native AOT | publish-and-run per claimed RID per composition, warnings as errors, closure report attached; execution-only and runtime-compiler evidence kept separate; suppressions inventoried with owner and reachability; **a continuous lane is not a bundle** — it publishes and runs the same roots on every RID and catches a regression between collections, and it retains no machine, no configuration and no output, so a green lane is a regression signal between collections and a bundle's evidence for nothing | an AOT claim derived from a property, an analyzer, or a non-AOT publish; a closure containing a test, reflection, or dynamic-code assembly; one composition's publish cited for another |
...
1. **Support truth:** the support table names the implemented and minimum-accepted core contract
   ...
   provided rather than left to be inferred from a second profile in the image**; **the amendment
   register's state is published row by row, with the deterministic failure or named exclusion each
   held row leaves standing and with the procedure's unexecutability stated**, because a held
...
| The extraction gate is never answered, and the same mechanism — a diagnostic registry, a conformance harness, an assurance floor, a bounded-read projection — is written twice and diverges on the first edit. | This profile supplies its half of the comparison when a second product profile's implementation has merged: file paths, source revision, and a correspondence table, recorded at the milestone that owns the mechanism where the condition is already met, **and at JS-10 regardless**. It records **no verdict**, because a verdict changes the core graph and because no identifier from another profile component may appear here. Where the second implementation has not merged, it records that the first condition is unsatisfied and names what would satisfy it. **The release is what makes the state recordable at all**: whether a second implementation has merged is a schedule this component does not hold, so every mechanism-owning milestone can pass with nothing to record, and a stop condition over an unrecorded state needs one point at which the state must exist *(corrected: JSC-32)*. **Stop: the verdict is the core architecture owner's and may be either, and an unsatisfied first condition is not a failure — but an unrecorded state is.** |
