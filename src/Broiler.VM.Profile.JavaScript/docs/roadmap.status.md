...
| **The conformance-suite revision is PINNED since 2026-09-03, and the pin is retained in this repository rather than in the checkout** *(the row read "is not pinned"; and the state it replaced was not merely transient but SELF-CERTIFYING — corrected: [JSC-68](roadmap.corrections.md#jsc-68))*. The pin is **test262 at `tc39/test262` commit `ccaac100ff49d81e9ff47a75ff4c60e0bd3f262e`**, content digest **`46d54f57ae3a4803c6ebc5f4625dd4b417254ed65058836732f182801e1cfe93`** over 56,560 files, held at [`src/tests/conformance/pins/`](../../tests/conformance/pins/README.md) where the suite cannot reach it; `--expect` makes a run answerable to it. Taken **twice**, the second retrieval extracted into a fresh directory and hashed independently of the first, and the two downloads byte-identical. [JSD-0020](decisions/0020-the-retained-conformance-suite-pin-and-the-one-it-replaces.md) records it. **Retrieved and hashed and NOT ARCHIVED**: 232 MB over 56,560 files, against the language specification's one file of 2.9 MB which was archived the same day — a decision about the shape of this repository rather than about evidence, recorded as one. The licence and attribution obligations are **not discharged and deliberately so**: section 14 lands the attribution row in the change that first ingests a suite FILE and this change ingests none, only the suite's licence text retained beside the pin. **Still open on the archiving action alone.** | This component's architecture owner | **What is left is one human action, where before it was three.** Until it is taken: **no floor is set over any test262 figure and none may be**, because ratcheting a number obtained against material somebody else can change would be a promise about a directory this repository does not hold — so every figure from that suite stays a measurement and not a commitment. **This is still what keeps JS-3a `In progress`.** |
