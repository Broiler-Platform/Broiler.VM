...
`[FULL]`, because no item's clauses are all met and the System V convention has executed nowhere —
...
| [PARTIAL] | **The two-forms check over the wide manifest** | **In progress**, as observed repository state | `NativeAbiChecks.TheTwoFormsAgreeOverTheWideManifest`, `ASwappedHandlerIsADefect` and `BaselineEntryPointsSurvive`; the conformance composition's native run under the wide manifest; `eng/compare-test262-forms.py`; rule `X3` and its witness. The bundle retains those rows passing under `x86-64-win64` and a whole-suite per-variant comparison of the two forms under one wall-clock allowance, across two builds and from one | Every row under `x86-64-sysv`, which only a Linux runner in the continuous-integration lane executes and which has not run; the bundle's other retained comparison, against a native run at a longer wall-clock allowance, which the comparison script does not pass because its differences in exhausted dimension are neither admitted nor classified (section 3 of the bundle); a like-for-like comparison under a deterministic allowance; the machine-code and bytecode workflow runs and their shard limits; the smallest completing fuel ceiling for every program that loads nothing; review |
