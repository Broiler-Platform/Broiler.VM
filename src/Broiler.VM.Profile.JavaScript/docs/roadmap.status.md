...
roots that publish on one RID under JIT, trimming and Native AOT — and **run clean in every mode
but one**, which the next paragraph is about — a retained corpus, a published diagnostic-code
...
- **No composition is advertised** and none is packable. Two composition roots exist, both
  ...
  claiming a RID is a release act and JS-10 owns it. **And "run" is not currently "run clean" in
  every mode**: the execution-only root's Native AOT run exits 1 on the soak's plateau check, which
  the JS-9 row states and which no row here reads past.
