...
**A backend roadmap exists in this checkout as of 2026-09-07, and it moves nothing in this ledger.**
[`roadmap.backends.md`](roadmap.backends.md) proposes what a second and third output form — an
artifact whose payload is machine code, executed in process, with the interpreter carrying
everything a backend declines — would take, and it mints its stages in a `JSB-n` namespace
[the delivery document](roadmap.delivery.md#19-milestones) does not hold and section 2's table does
not read. **This paragraph is observed repository state under section 1's second category and is not
a status of any kind**: a document existing is a fact about the checkout, and the status vocabulary
below says in its own words that planning text does not change a state. No row in section 2 moved
when it was written, none is cited by it, and none of its stages has an owner — which is the
property that distinguishes a stage from a milestone rather than a formality about it, and the same
property the two proposal documents beside it already carry.

**That paragraph continued, until 2026-09-07, by saying that this profile owns no backend code of
any kind — no instruction encoder, no executable mapping, no emitted byte, no artifact section that
could carry one — and every clause of it has since become false.** It is replaced rather than edited,
because a reader who met it is owed the retraction and because the sentence that replaces it is
longer than the one it replaces.

**What this profile now owns, recorded as observed repository state under section 1's second category
and satisfying no gate** *(added 2026-09-07)*. A third feature manifest, `broiler.javascript.numeric`,
whose admission pass refuses every construct outside a numeric subset **at compile time, by name**,
with a diagnostic code and a source position. Two section kinds in format version 2 — the emitted
bytes with their architecture, backend version and alignment, and a symbol row per code unit — read
by the same verifier that reads every other kind, with the ordinary bytecode written in full beside
them in every artifact. An optional surface identity for the native payload, which a composition
declines the way it declines the binary and module surfaces, refusing an artifact that names it **at
verification**. Instruction encoders in the lowering assembly for two x86-64 calling conventions and
for arm64. One type in the profile assembly — and only one — that reserves a mapping, writes it and
then arms it readable-and-executable, never both at once. And a verifier arm that recompiles an
artifact's own bytecode with the same deterministic backend and requires the emitted bytes to match,
available to an image that carries the lowering and to no other.

**The form is whole-artifact and there is no fallback anywhere in it**, which is the property that
keeps this profile's amended non-goal literal: an artifact is native or it is bytecode, every code
unit of a native artifact is emitted, and an artifact whose architecture the running process is not
**refuses to instantiate** rather than running the bytecode beside it. The design and the rule that
forces it are [the backend roadmap's](roadmap.backends.md) section 3; the route taken in place of the
decision that would have chosen it is
...
publish-and-run under any mode, no retained corpus replay, no negative control watched failing, and
no human has read a line. *(Corrected 2026-09-15, after collection: that sentence is still true of the
numeric form. For the wide-manifest form the code is committed, and
[bundle JSB-11-001](evidence/jsb-11-001/README.md) retains a publish-and-run in three modes and a
corpus replay; no negative control was watched failing and no human has read a line of it either:
[JSC-223](roadmap.corrections.md#jsc-223).)* The backend roadmap's own stages say, one by one, which clauses the code
meets and which it does not, and **owning code is not acceptance**.

**One figure exists about it and this ledger does not carry it, which is update rule 10 doing exactly
what it is for.** A native artifact was observed to run faster than the bytecode form of the same
source, on one machine, once, with no A/A lane, no predeclared rule, no retained repetitions and no
benchmark host. **That is not a measurement**, `JS-10` owns what would make one, and no number from
...
**A second native form, over the wide manifest, was written on 2026-09-15 and a bundle retains part of
what its stage asks, recorded as observed repository state under section 1's second category and
satisfying no milestone gate** *(added 2026-09-15; revised the same day after collection)*.
...
roadmap's](roadmap.backends.md) stage `JSB-11`, whose State bullet names clause by clause what the
bundle meets and what it does not. *(Revised 2026-09-15, after collection. This paragraph read "What
the checkout holds of it on that date is four declarations in the format assembly — `JsBaselineFrame`,
`JsBaselineAbi`, `JsBaselineStatus` and `JsNativeTier` — and the rest is being written", and said that
each row below was marked `[NONE]` because no bundle retained anything about it. The code was committed
on that date and [bundle JSB-11-001](evidence/jsb-11-001/README.md) was collected from it on one
`win-x64` workstation.)* **No milestone row in section 2 moves for it**, for the reason the paragraphs
above give for the numeric form: `JS-1`, `JS-3b` and `JS-5` own the format, the lowering and the
executor, and none of their exit gates asks for an output form. The three items below are set out as
rows so that each names what it lacks. **Each is marked `[PARTIAL]` because that bundle demonstrates
some of what the stage asks of the item and names every unmet clause in its own exclusions** — none is
`[FULL]`, because no item's clauses are all met *(corrected 2026-09-15: this also read "and the System
V convention has executed nowhere", which a machine-code workflow run and a lane job on hosted Linux runners
have since made untrue: [JSC-224](roadmap.corrections.md#jsc-224))* —
and the state column is observed repository state and not a milestone state. **A `[PARTIAL]` item is
not a qualified pass: the bundle is not acceptance, one machine ran each thing once, and no human has
read a line.**

| Verdict | Item | Observed state | What the checkout holds | What is open |
|---|---|---|---|---|
| [PARTIAL] | **The baseline form: emitted units calling the interpreter's own dispatch, one instruction per call** | **In progress**, as observed repository state | The frame, ABI constants, status and tier declarations beside `JsBaselineFrame`; `JsEngine.ExecuteCore` as the interpreter's body specialised over a mode; `JsNativeActivation` and its handler step; `JsBaselineHandlers` and its table; the engine's native entry `RunNative`; the compile path through `JsCompiler.Emit`; and the x86-64 backend's baseline emitter, `JsX64BaselineEmitter`. [Bundle JSB-11-001](evidence/jsb-11-001/README.md) retains, on `win-x64`: a whole-suite conformance run in this form compared variant by variant with a bytecode run; a whole-suite bytecode run from the collected build whose merged report is byte-identical to the merge base's; the call-depth measure in both forms; the interpreter's JIT summary either side of the change; and a Native AOT publish and run of all four roots. From hosted Linux runners it also retains a whole-suite workflow run in this form under `x86-64-sysv` | Every clause that bundle names as unmet: tolerances predeclared before the runs they judge, without which the retained benchmark reports, the retained before-and-after bytecode run over named subtrees and the frame-cost measures at the merge base meet nothing; those runs taken from the collected commit; the stack reservation; each per-step instantiation's size and jump tables; the fresh-process cost; a fresh process recursing to the ceiling; the native margin; a predeclared bound for each image's growth against the merge base, which the bundle records; the interpreter audit the stage requires; the `x86-64-sysv` checks rows named as passed in a retained transcript; review |
| [PARTIAL] | **The baseline scan: the wide manifest's template tables and shape clauses** | **In progress**, as observed repository state | `JsNativeTier`, which selects a table; the baseline tables behind the tier-aware `JsNativeTemplates.For`; the tier-aware `JsNativeScan.Scan` with its shape clauses; the verifier's choice of table by manifest; rule `X2` and its witness. The bundle retains the slice compiler's closure, coverage, refusal, retained-byte, cross-convention and re-emission rows passing in JIT, trimmed and Native AOT images, and the corpus replaying with its entry for a baseline payload calling a slot no opcode takes | A refusal row for a call past the table that the transcript names as such; the verifier's reason shown on each refusal; a hand check of the retained bytes against the templates; an injection into the tree watched failing and passing after revert for rule `X2`; review |
| [PARTIAL] | **The two-forms check over the wide manifest** | **In progress**, as observed repository state | `NativeAbiChecks.TheTwoFormsAgreeOverTheWideManifest`, `ASwappedHandlerIsADefect` and `BaselineEntryPointsSurvive`; the conformance composition's native run under the wide manifest; `eng/compare-test262-forms.py`; rule `X3` and its witness. The bundle retains those rows passing under `x86-64-win64` and a whole-suite per-variant comparison of the two forms under one wall-clock allowance, across two builds and from one. It also retains the machine-code and bytecode workflow runs of the conformance suite on one commit, with every shard job inside its limit, and their per-variant comparison under the comparison script both as it stood at their commit and as widened after them; and selected lines of a pull-request lane job on a Linux runner | Every row under `x86-64-sysv` named as passed in a retained transcript, which that lane job's transcript does not print; the differences, in the workflow pair and in the comparison against a native run at a longer wall-clock allowance, that are wall-clock exhaustions in the bytecode form against another allowance's exhaustion in the native form — the comparison script admits them only as widened after the pair was seen, and the stage's gate names them in no class, so it calls them defects ([JSC-224](roadmap.corrections.md#jsc-224); section 3 of the bundle); a like-for-like comparison under a deterministic allowance; the smallest completing fuel ceiling for every program that loads nothing; review |

**No figure about this form is written in this ledger, and none is to be**, under update rule 10:
what the stage's bundle retains belongs to the bundle, and a reader who wants a number reads it there.
...
Until that date the wide front end **admitted a BigInt literal and evaluated it as a Number**:
`typeof 1n` answered `"number"`, `1n === 1` answered `true`, and `9007199254740993n` answered
`9007199254740992` — an integer no reader would have any reason to doubt. It reached the armed
native path too, where `--numeric --native x86-64-win64` emitted machine code returning the same
wrong integer; `--slice` alone refused it by name. The wide parser now reads the `n` suffix the
tokenizer already leaves on the token's raw text and answers
`2104:ConstructOutsideManifest … a BigInt literal is not admitted by the declared feature manifest`.
**The refusal is restored and the value kind is not implemented** — there is still no BigInt in this
realm, and the absent-globals block below is unchanged. **This is a defect this component had
already written down and not acted on**: it is the parity roadmap's
[section 4.2](roadmap.parity.md#42-the-refusal-that-was-lost), recorded on 2026-09-06, and
[JSC-207](roadmap.corrections.md#jsc-207) carries the correction with everything it cost.

**What it cost is a fall in the pass column, and the ledger states it rather than letting the next
run state it.** Over `test/language/literals/bigint` the answer moved from **118 variants: pass 101,
fail 17, unsupported 0** to **118 variants: pass 58, fail 0, unsupported 60**, measured either side
of the change on this checkout by `eng/run-test262.py` over that subtree — a subtree somebody named,
so a fact about that subtree and not about the suite. **So the whole-suite `passed` total falls and
the wide run's `unsupported` column stops being empty**, and that is the trade this component's own
rules ask for: a refusal that names the construct is a supported answer and a plausible wrong value
is not, which is what the fourth verdict exists to say and what
[JSC-54](roadmap.corrections.md#jsc-54) and [JSC-66](roadmap.corrections.md#jsc-66) already measured
the price of reading past. **A smaller pass total that is honest beats a larger one that is not.**

**The run has been taken, and the sentence that stood here promising it is replaced by its answer**
*(replaced 2026-09-08)*. What this paragraph read a few hours earlier was *“No figure for the run
after this change exists here, because none has been taken. A fresh whole-suite run is being taken
and its totals are not known”*, and leaving that standing once the answer arrived is exactly the
stale-summary failure update rule 1 exists against. **The run is whole and it is pinned**:
...
from this working tree, with the report's coverage field reading `whole` and the report's own line
saying that this run may be retained — pinned, whole, and its verdicts account for it.
**53,469 files, 94,545 variants: pass 70,834, fail 13,315, unsupported 1,990, exhausted 60,
skipped 8,346.** Update rule 10 is the reason every one of those numbers is written with the
command, the pinned revision, the date and the runtime identifier fastened to it: a total that
travels without them is one a later reader carries forward into a tree it never described, and this
one describes 2026-09-08 on `win-x64` and nothing else.

**What moved, and against what.** The comparison is the run of 2026-09-07 taken **before** the
BigInt refusal was restored — pass 71,153, fail 14,986, unsupported 0 — and against it this run is
**319 fewer passes, 1,671 fewer failures, and 1,990 newly unsupported**. **Every one of the 1,990 is
the BigInt literal meeting a refusal that names it.** The two movements sum to exactly the newly
unsupported count, which is what one change and nothing else moving looks like; it is not a proof of
it, because two offsetting movements elsewhere would look the same from these five numbers. **The
fall in the pass column is larger than the subtree measured either side of the change**:
`test/language/literals/bigint` accounted for 43 of the 319, and the remainder is BigInt literals
sitting in files that are about something else. **The largest declared-feature rows among the
remaining failures are TypedArray, iterator-helpers, regexp-unicode-property-escapes, BigInt
(1,030), resizable-arraybuffer, SharedArrayBuffer, Symbol and Atomics** — and that BigInt row is now
tests that **use** a BigInt value reaching them from elsewhere rather than tests carrying a literal,
which is the shape a refusal at the literal would be expected to leave behind. **So the fall this
paragraph predicted above is measured rather than predicted now**, and the trade is the one this
component's rules ask for: 1,990 variants that answered plausibly-wrongly or failed silently now
meet a refusal that names the construct, and the price of that is 319 passes.

**The floor was re-based by hand, and the fact that `--admit` could not do it is the ratchet working
rather than a tool that fell short.** The floor is
[`test262-wide.floor`](../../tests/conformance/floors/test262-wide.floor); `--admit` re-bases only
when the suite revision or the manifest moved, and **neither moved here** — the engine did — so the
ratchet answered `Regressed` and refused, which is precisely what a ratchet over third-party
material is for. The re-base was therefore a deliberate hand edit rather than a tool's: the new
floor **holds** against the new run, verified by the lane's own answer that *the test262 floor
holds*; the superseded rows are retired in the file with the reason they were retired; and the
file's header now states **in writing** that a lower `passed` row is not a regression here, and why.
The component's own gate is green at the same commit — Contract 207/207, Architecture 221/221 —
which says nothing about the suite and is recorded so that no reader infers a broken tree from a
smaller pass total.

**No row's evidence column cites any of this, and the reason is the ledger's own rule rather than
caution.** [Section 4](#4-required-evidence-bundle) requires every status claim beyond `Not started`
to point to a **retained bundle** carrying identity, source, dependencies and corpus, environment,
procedure, raw results, negative controls, closure and exclusions; update rule 2 says to link that
immutable bundle and state what it demonstrated. **This is a working-tree run and no bundle has been
collected from it** — no environment record, no negative controls, no retained raw outputs, no
reviewer — so it is [section 1](#1-reading-this-ledger)'s second category, observed repository
state, dated and on one runtime identifier, and nothing more. Quietly citing it in an evidence
column would put a working-tree run where rule 2 asks for an immutable bundle, and reading a single
machine's answer as more than that is what rule 3 forbids, so this ledger states the rule out loud
rather than acting on the figures: **the figures a row here may cite are still [bundle JSW-10-001](evidence/jsw-10-001/README.md)'s,
and every one of them describes a run taken on 2026-09-05, before this change**. No byte of that
bundle is edited and nothing in it stopped being true — the checkout moved under it, which is the
ordinary way a retained figure stops describing a tree and the reason a bundle is dated. **No row
...
Nine rows are `[PARTIAL]` and the remaining three are `[NONE]`. **Two moved on 2026-09-05**, when the workload programme retained [bundle JSW-10-001](evidence/jsw-10-001/README.md): `JS-7` and `JS-8` had read `Not started` while the suspension families and all four guest-load shapes were built and exercised by a third-party corpus, which is the row going stale in the direction this ledger's update rule 1 exists against.
...
literal is refused **by name at the front end**, which puts `BigInt` on both lists at once and is the
only entry here of which that is true. [JSC-207](roadmap.corrections.md#jsc-207) carries it.
...
- **No language is supported.** Two feature manifests exist and **neither is accepted**: acceptance
  ...
  here.
- **No composition is advertised** and none is packable. Every JavaScript composition root
  [the register](../../../docs/compositions.md) names is registered as a demonstration, and this
  bullet no longer counts them *(corrected 2026-09-07: it said three, naming the slice compiler, the
  execution-only image and the Android head, and it had been wrong since the conformance and CLI
  roots were added — a count copied into prose, which update rule 10 forbids for this exact reason)*.
  **Each of them declares `x86-64` in the register's native-execution column from 2026-09-07**, because
...
- **The product code that exists is a slice and a wide surface beside it, and each says which it
  ...
  library, and a second source front end sharing the first's tokenizer and nothing else. **And since
  ...
  [JSC-220](roadmap.corrections.md#jsc-220); corrected again 2026-09-15, after collection: that form
  has landed, so "every native artifact" is no longer true of the checkout, and
  [bundle JSB-11-001](evidence/jsb-11-001/README.md) retains wide-manifest native artifacts emitted and
  run on one `win-x64` workstation, which is not acceptance:
  [JSC-223](roadmap.corrections.md#jsc-223))* — **the verifier's own gate on a machine-code payload is the
  ...
  above have contradicted since 2026-09-05: suspension exists and all four load shapes exist. The
  clause is withdrawn here rather than carried, and those two rows are the authority — corrected
  2026-09-07.)* The representation JS-4's entry gate fixed
...
|---|---|---|
| **OPENED 2026-08-31.** Acquisition of the regular-expression matcher and the Unicode and locale data as this checkout's own dependencies. **Owner: the profile built-ins owner**, named in [JSD-0005](decisions/0005-the-seed-waited-on-set-and-snapshot-stop-condition.md). Nothing is acquired yet; what changed is that the dependency now has a holder. **Still unacquired on 2026-09-04, and the milestone that consumes it landed anyway.** | Opened at JS-0, consumed at JS-6 | JS-6 excludes every surface needing it and publishes the exclusions, rather than waiting. `broiler.javascript.regexp` is already a separate manifest identity, so the exclusion is a manifest not yet minted rather than a hole in one that is. **Neither of those two routes is the one taken.** JS-6 landed on 2026-09-04 with regular expressions **translated onto `System.Text.RegularExpressions`** — inside `broiler.javascript.wide` rather than behind a manifest identity of their own, declared as an approximation in the file that makes it, and with the Octane RegExp benchmark running and failing its own checksum — and with `Date` fixed to UTC where the locale half of this dependency would have been consumed. **An approximation was a third state this row did not anticipate**: it neither waited nor excluded, and what it cost was a surface that answered wrongly instead of refusing. **A FOURTH STATE IS WHERE THE MATCHER HALF ACTUALLY ENDED, and it is neither of the two routes nor the approximation**: the workload programme wrote a matcher in this checkout, so the surface is IMPLEMENTED rather than acquired, and rule **N18** asserts over the product source that no call site constructs a compiled-mode regular expression from the platform — because a matcher landing is a state of the tree a later `using` could undo with every test still green. **What is still unacquired is the rest of the row**: the Unicode and locale data, with `Date` still fixed to UTC where the locale half would have been consumed. **The route is corrected nowhere** — JS-6's row still asks for the matcher as an ACQUISITION and for the separate manifest identity, and regular expressions are still inside `broiler.javascript.wide` rather than behind `broiler.javascript.regexp`, and [JSC-70](roadmap.corrections.md#jsc-70) records this manifest admitting a `RegExp` only as the second of its two reasons the manifest could not be called `core`, on decision [JSD-0021](decisions/0021-the-wide-bring-up-manifest-and-format-version-2.md). The dependency itself is **still open** and still has the holder JS-0 gave it. |
