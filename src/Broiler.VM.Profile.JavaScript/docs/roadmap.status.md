...
| [NONE] | **JS-10 — baselines, packaging, support table, release gate** | **Not started** | None. No measurement lane, no baseline register, no package, no support table, no human review decision on anything. **The language-specification edition is pinned provisionally since 2026-09-03 and the conformance-suite revision is not pinned at all** *(this cell said neither was pinned; corrected: [JSC-67](roadmap.corrections.md#jsc-67))*, and roadmap [section 24](roadmap.gates.md#24-specification-and-platform-references) requires a provisional pin to carry a named exclusion in this ledger until a human has retrieved, hashed, and archived it — see section 3. **Gate 1's support table needs both by immutable identifier**, so a provisional pin narrows what this row waits on and closes none of it. | After JS-9, and after a named human has read every relevant unit — which is the largest single-owner task in the programme and must be scheduled, not assumed. |
...
### The language edition is pinned, provisionally, and it checked three claims and found one disagreement
...
three, and the middle one is exactly this: **retrieved and hashed and not archived is a PROVISIONAL
pin carrying a named exclusion.**
...
**It accepts nothing.** No manifest may be accepted against an unpinned edition and a provisional
pin is not the fixed one; nothing is archived; no third-party document is in this repository.
...
one. **JS-0 opened two of them and left two unopened; a third was opened on 2026-09-03** when the
language-specification edition was pinned provisionally, which gives that row a holder and an
unblock condition it did not have. The table says which is which:
...
| **The language-specification edition is PINNED PROVISIONALLY since 2026-09-03, and this row is the named exclusion that provisionality requires** *(corrected: [JSC-67](roadmap.corrections.md#jsc-67) — it read "not pinned, and JS-0 did not pin it… no decision record does")*. The pin actually taken is **ECMA-262, 17th edition (ES2026)**, at `tc39/ecma262` commit **`0248456c758431e4bb8e5d26333ff1865123c9cd`** — the commit the `es2026` tag resolves to, pinned as a commit because a tag can be moved — with `spec.html` of 2,978,793 bytes hashing to **`ce7bc30174061fd8d212270b81cf6511661180c1e174f6911d10ced0581527b0`**. [JSD-0019](decisions/0019-the-pinned-language-edition-and-what-two-of-three-actions-buy.md) records it, why the errata revision was refused, and how a reader re-derives the digest in one line. **Two of section 24's three actions are done: retrieved and hashed, NOT ARCHIVED**, and the document is in no directory this repository holds. That is what keeps the pin provisional and this row open. | Archiving is this component's architecture owner's; the pin itself is taken | **A named human archives the retrieved document at that revision.** Until then: no manifest may be accepted against it — a conformance total against a moving document is not a total, and a provisional pin is not the fixed one — and the profile's own declaration says `Archived = false`, which rule **N14** holds this row's wording to in both directions. |
