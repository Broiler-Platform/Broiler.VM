...
| `broiler.javascript.modules` | Module records, live bindings, import and export forms, and top-level await, which IS declared: the surface was allocated "where declared" against a profile that had neither `async` functions nor a job queue, and the one that opened it has both *(corrected: JSC-128)*. | Opened by JSW-8 |
