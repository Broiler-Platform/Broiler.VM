...
**This bundle supports `In progress`. It does not support `Accepted`.** No code unit carries a
decision on its `// Broiler-Human:` line, so the generated `HUMAN_REVIEW.md` states `PENDING` and
no milestone here may be accepted, no package published and no RID
claimed.
...
| Environment | 4 | [PART] | One RID on one developer workstation for everything this bundle retains: no arm64, no macOS, no second machine (EX-45). The two lanes added at VM-1 build and test on `ubuntu-latest` and reproduce none of the results here. |
| Procedure | 5 | [MET] | - |
...
| `test.log` | **221 passed, 0 failed, 0 skipped** - 90 architecture and 131 behavioural |
...
| `rules.register.json` | 54 rules: **49 Active, 1 Vacuous, 4 Deferred** |
...
Nine entries have been corrected after collection. Two were figures that disagreed with the logs
this bundle retains. The other seven describe a checkout file rather than a log, and each changed
after the logs were taken rather than having been wrong when written. All nine are corrected above
...
| 2026-08-28 | Section 6, the `rules.register.json` row, a sixth time | 52 rules, 47 of them Active | 53 rules, 48 of them Active | The register itself. The falsification criteria moved out of the reviewer worksheet and into the source: the annotation gained a third line, `// Broiler-Falsified-If: <prose>`, between the AI line and the human line, and rule J10 requires it on every unit assessed `Security=High` or `Critical` and names each one that carries none. The worksheet held those criteria in a document whose citations had already rotted - RC-01 cited a checked multiplication by line number, at a line the annotations themselves had turned into a parameter declaration - and an annotation travels with its declaration where a line number does not. The line is a comment, so it is outside every fingerprint and no review is bound to it. It is an extension to `BROILER-CODE-ASSURANCE.md` rather than an implementation of it, which is EX-74, and nothing judges a criterion, which is EX-75. Same recertification trigger as the five rows above. |
| 2026-08-28 | Section 6, the `rules.register.json` row, a seventh time; EX-45, EX-55, EX-60, EX-74, EX-75 and EX-77 | `53 rules, 48 of them Active`, and a review recorded in a hand-written `HUMAN_REVIEW.md` beside a per-item worksheet | `54 rules, 49 of them Active`, and a review recorded on the declaration itself | The register itself, and the tree. The owner ruled on 2026-08-28 that a reviewer fills in the source comment and nothing else. `HUMAN_REVIEW.md` is now a generated artefact in the generator's plan, derived from the human lines and the per-file headers, so it carries a row for every alias the tree names rather than one signature block and records no commit - a decision is bound to the fingerprint of its declaration, which is what the ruling replaced a revision with. `docs/review/vm-0-vm-1.md` is deleted; EX-55 closes with it. Rule J11 is new: the release gate, armed by `BROILER_ASSURANCE_RELEASE=1`, refusing a publish while any artefact is stale, any annotation unreadable, any fingerprint out of date, any `High` unit unfalsifiable or any relevant unit unresolved. Rules H3 and H4 are restated against the new mechanism rather than retired - H3 now refuses a source line number anywhere in any review document, where it reached only the worksheet's `Read` rows before, and holds the record's coverage table to the covered file set; H4 holds the record's status word and alias rows to the human lines in both directions. Two GitHub Actions lanes were added, which narrows EX-60 and adds EX-79. **Four rules asserted this milestone's state as though it were a rule, and each would have turned the suite red on the first line a reviewer wrote:** J4 required every human line in the tree and in every artefact to read `PENDING`; J5 and J9 compared generated figures against literal zeros; J9 took its supporting count over the whole component while a file header states its own file's, so one recorded decision would have made 44 truthful headers into violations; and the report rendered `state VERIFIED` as bare prose, which J9 reports. All four are now derived from the annotations. Same recertification trigger as the six rows above. |
| 2026-08-28 | Section 9, EX-74 and EX-75 | `three of the forty-four units that owe a criterion carry one, and J10 names the other forty-one` | `sixty-nine units carry a criterion, the forty-four that owe one among them, so J10 names nobody` | The tree, and the *Falsification criteria* section of `CODE-ASSURANCE.md`, which is generated from it. The migration is finished: the code-only worksheet items that had not yet found a declaration - the metering chain, the retain-and-release pair, the wall clock, the outcome mapping, the instantiation stage, the placeholder instance, the lease contract and the `Snapshot` copy - are criteria on the units they were written about, which are assessed `Security=Medium` and so carry them by permission rather than by requirement. With none outstanding, `docs/review/vm-0-vm-1.md` was cut to what is not a code unit: fifty-three items became thirty-five, the twenty-two code-only ones deleted and one added per code area asking the reviewer to judge the criteria that replaced them. EX-74's account of the worksheet moves to the past tense for the same reason, and `HUMAN_REVIEW.md` section 4 states the new count. Same recertification trigger as the six rows above. |

No correction changes a log, and no log was re-collected. The first two make the prose agree with
evidence that was already retained here; the last seven record changes to the checkout the bundle
...
`test.log` records **221 passed, 0 failed, 0 skipped - 90 architecture and 131 behavioural**
...
| EX-45 | Open | **One RID, one machine.** The retained build, test, pack and publish results in this bundle are `win-x64` only, on a developer workstation: no arm64, no macOS, no second machine. Two GitHub Actions lanes were added on 2026-08-28 and they change less than they look like they change - they run `dotnet build`, `dotnet test` and the two assurance modes on `ubuntu-latest`, and they reproduce none of the evidence here. The Native AOT result in particular is still a single manual run (EX-42). |
...
| EX-53 | Open | **The group H and group J rules have a nominal owning record, not a real one.** All fifteen rows in `rules.register.json` name ADR 0012 as `owningAdr`: group H because it owns the support-claim and ownership surface those rules protect, group J because ADR 0012 owns who is allowed to say yes, which is exactly what the Broiler Code Assurance system records. ADR 0012 is frozen and names neither group. Every other row in the register names a record that states its rule, so these fifteen are the places where `owningAdr` asserts less than it looks like it asserts, and a reader should not read it as a binding the record itself carries. Group J is further out on this limb than group H: its authority is `BROILER-CODE-ASSURANCE.md`, an owner's policy document that is not an ADR of this component at all, so no frozen record in `docs/adr/` states the rules J1 to J10 enforce, and J10 goes further still: it enforces a line the owner's policy does not define at all, which is EX-74. Closed by: the first amendment that lets a record state them, or a record of their own. |
...
| EX-55 | Closed | **Closed 2026-08-28.** It recorded that rule H3 did not fix how many items the review worksheet must carry: the count was held by agreement between two documents, so a coordinated edit to both could shed a checklist item with the suite green, and nothing outside those two documents said how large the review ought to be. The worksheet is deleted and the extent of the review is now the tree - H3 holds the generated record's coverage table to `AssuranceSources.Files` and to the annotations, neither of which is a document anybody edits to change the answer. Retained as dated history: the limit was real while the worksheet existed. |
...
| EX-60 | Open | **Narrowed 2026-08-28: two lanes now run both assurance modes, and what a lane cannot do is what is left.** The review lane regenerates every artefact on a pull request and commits what moved; the publish lane runs the gate and then the release gate, which refuses a publish while any relevant unit is unresolved, any fingerprint is out of date, any annotation is unreadable or any generated artefact is stale. Three things remain open and none of them is closed by having a lane. A lane fires on a pull request and on a tag, so a commit pushed to a branch nobody opens a pull request for is examined by nothing until the next one; no protected-branch rule is configured here, and configuring one is a repository setting rather than a file in this checkout. Nothing compares a human line against its parent commit to decide whether a transition was legitimate. And nothing binds an alias to a hosting identity: the record holds an alias to appearing in the tree, and neither rule H4 nor rule J4 can tell a real person from a plausible string. **The original text, retained:** `BROILER-CODE-ASSURANCE.md` assigns fingerprint maintenance and generated-summary maintenance to CI, and makes an unresolved assurance state a release blocker. This component has no CI lane at all - EX-45 records one RID, one machine and no lane - so the generator and the gate are the same code run as a test in the architecture suite: `BROILER_ASSURANCE_WRITE=1 dotnet test Broiler.VM.slnx -c Release` writes every fingerprint, header and `CODE-ASSURANCE.md`, and a bare `dotnet test Broiler.VM.slnx -c Release` asserts that what is on disk is byte-identical to what that run would produce. Rule J5 is that gate and it is real, but nothing external compels it: no protected branch, required review or workflow refuses a commit whose annotations have gone stale, and a developer who does not run the suite is stopped by nothing. Two further things the policy asks for cannot exist without a lane and do not exist here: comparing a change against its parent commit to decide whether a review transition is legitimate, and binding a reviewer declaration to a GitHub identity and pull request. Closed by: a CI lane that runs both modes and blocks the merge on the gate. |
...
| EX-74 | Open | **The third annotation line is an extension to the owner's policy, not an implementation of it, and the owner may reject it.** `BROILER-CODE-ASSURANCE.md` defines two lines above a code unit: the AI assessment and the human verification. This component writes a third between them - `// Broiler-Falsified-If: <prose>`, one sentence naming what would make the unit wrong - and rule J10 requires it on every unit assessed `Security=High` or `Security=Critical`. Two things made it necessary and neither is a defect in the policy. The two lines the policy defines cannot carry a falsification criterion at all: `Security=High` names a set of forty-four units, which is not a test, and the test is what a reviewer standing at the declaration needs. And the document that did carry the criteria, the reviewer worksheet `docs/review/vm-0-vm-1.md`, cited line numbers - its `RC-01` cited the checked multiplication in `VmBoundedAllocator.TryAllocate` by line number, and the line it named became a parameter declaration when the annotations moved the code; rule H3 checked identifiers, areas and counts and never whether a cited line meant anything. An annotation travels with its declaration; a citation does not. The line is a comment, so it is outside every fingerprint by construction and no review is bound to it, which is asserted by rule J10 rather than argued. The worksheet is deleted outright as of 2026-08-28, and with it the last place a criterion could have gone back to: `HUMAN_REVIEW.md` is generated from the annotations, and the criteria are echoed into its section 9 beside the units that carry them, so the record shows a reader the observation and the human line together. Closed by: the owner adopting the third line, or rejecting it - in which case the criteria have to be written again into a document, and the rot comes back with them. |
| EX-75 | Open | **Nothing judges a falsification criterion.** Rule J2 holds the line to parsing where the block is, to saying something, to saying it on one line and to carrying no `Key=Value` field; rule J10 holds every `High` and `Critical` unit to carrying one. Neither can read it. A criterion reading `the code is wrong` satisfies both completely, and so does one that names an observation about a different unit, or one that restates the assessment it sits under - which is the reason the line is required above High and merely permitted below it, because a criterion written to fill a slot is worse than an absent one. This is the shape of EX-68's limit on the per-unit exemption reason: a rule can hold a sentence to being present and cannot decide whether it is true. What narrows it is provenance rather than a rule: every criterion in the tree is carried across from one of the twenty-two code-only items the reviewer worksheet used to carry, whose prose was written against this code in the adversarial review, or written in that voice where a `High` unit had no worksheet item. Sixty-nine units carry a criterion, the forty-four that owe one among them, so J10 names nobody - and provenance is not judgement: no human has read a single one of them. Closed by: a human review of the criteria themselves, which is the thing this whole system records the absence of. That review is now asked for where the criterion is: section 9 of `HUMAN_REVIEW.md` lists every unit at the top of the security vocabulary with its criterion and its human line beside it, so a reader judging the criterion and recording a decision are the same act in the same place. |
| EX-76 | Open | **A downgraded assessment releases the criterion it required.** Rule J10 requires a falsification criterion on every unit assessed `Security=High` or `Critical`. Editing a unit from `High` to `Medium` and deleting its criterion leaves the whole suite green in both modes, because an assessment is a comment (EX-65) and moves no fingerprint, and because the requirement is keyed to the assessment rather than to anything independent of it. Nothing mechanical can judge whether a unit is High; a per-assembly floor would close the largest instance - `Broiler.VM.Binary` decodes untrusted bytes and arguably has one - and is not implemented. The change is visible as a diff in `assurance.manifest.json` and in the annotation, and is caught by review or not at all. |
| EX-77 | Open | **Fifteen of the sixty-nine criteria are new prose, not migrated prose.** The criteria were meant to carry across the wording of the twenty-two code-only items the reviewer worksheet used to carry, whose text was the product of real thought about this code. Most did. Fifteen did not, because the units they sit on had no worksheet item - the largest block being the nine on `VmProfileId`, seven of them on `High` units, where the worksheet named nothing at all. The worksheet is now deleted, so this exclusion is the only surviving record of which criteria were migrated and which were written. Those fifteen have had no more scrutiny than any other sentence written in this change, and a reviewer should read them as new claims rather than as reviewed ones. |
| EX-78 | Closed | **Closed 2026-08-29 by amending ADR 0001.** It recorded that ADR 0001's artefact register described `HUMAN_REVIEW.md` as *a scoped attestation of a reviewed revision* under the binding rule that no package is published *without a completed review naming the reviewed commit*, while the record had become generated and bound each decision to a declaration fingerprint instead. **The pointer this exclusion carried was wrong and is corrected here:** it named the amendment procedure in ADR 0003 section 6, which mints a core contract version and governs the contract-bearing ten. ADR 0001 declares `not contract-bearing`, and `docs/adr/README.md` states that the two records which are not contract-bearing govern component shape and ownership instead - so no version is minted and the record is revised directly, under ledger update rule 1, which asks that earlier decisions be preserved as dated history. ADR 0001 now carries a `Revisions` section stating what each amended row said before. The amendment reached two rows this exclusion did not name: `.github/workflows/`, which still read *deferred to VM-6* and *the component runs no CI of its own*, and the absence of any row at all for `CODE-ASSURANCE.md` and `assurance.manifest.json`. Retained as dated history: the divergence was real from the moment the record became generated until the record was amended. |
| EX-79 | Open | **The review lane trusts the branch it commits to.** It regenerates every assurance artefact on a pull request and pushes the result to the head branch, which means a change to the generator, to the shape, or to an annotation is regenerated and committed by the same run that was asked to check it. Nothing in that loop is a second opinion: the gate it then runs asks whether the tree agrees with the generator, and after a regeneration it does by construction. What limits the damage is that the lane cannot invent an approval - `RefuseInventedApproval` throws and rules J4 and J9 read the rendered artefacts - and that the diff is on the pull request for a human to read. A fork's pull request is not committed to at all: the lane fails and names what is stale. Closed by: a required review of the lane's own commits, which is a repository setting rather than a file here. |
