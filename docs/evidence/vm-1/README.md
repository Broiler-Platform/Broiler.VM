...
| EX-78 | Open | **ADR 0001's artefact register describes a document this component no longer produces, and the record is frozen.** Its row for `HUMAN_REVIEW.md` calls it *a scoped attestation of a reviewed revision* and states the binding rule *no Broiler.VM package is published without a completed review naming the reviewed commit*. As of 2026-08-28 the record is generated from the annotations, carries no attestation block and names no commit: a decision is bound to the fingerprint of the declaration it was made against, which is narrower than a revision and is what the owner ruled should replace it. The record is `Proposed` and unapproved, so this is recorded as a deviation rather than made by editing a frozen record. Closed by: an amendment to ADR 0001's artefact register under the procedure in ADR 0003 section 11, or a decision to reverse the change. |
