...
| 2026-08-28 | Section 6, the `rules.register.json` row, a sixth time | 52 rules, 47 of them Active | 53 rules, 48 of them Active | The register itself. The falsification criteria moved out of the reviewer worksheet and into the source: the annotation gained a third line, `// Broiler-Falsified-If: <prose>`, between the AI line and the human line, and rule J10 requires it on every unit assessed `Security=High` or `Critical` and names each one that carries none. The worksheet held those criteria in a document whose citations had already rotted - RC-01 cited a checked multiplication at `VmBoundedAllocator.cs:57`, which is now a parameter declaration - and an annotation travels with its declaration where a line number does not. The line is a comment, so it is outside every fingerprint and no review is bound to it. It is an extension to `BROILER-CODE-ASSURANCE.md` rather than an implementation of it, which is EX-74, and nothing judges a criterion, which is EX-75. Same recertification trigger as the five rows above. |
