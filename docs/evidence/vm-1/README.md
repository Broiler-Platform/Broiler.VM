...
| `rules.register.json` | 43 rules: **38 Active, 1 Vacuous, 4 Deferred** |
...
Three entries have been corrected after collection. Two were figures that disagreed with the logs
this bundle retains. The third is a count of a checkout file, not of a log, and it changed after
the logs were taken rather than having been wrong when written. All three are corrected above and
...
evidence that was already retained here; the third records a change to the checkout the bundle
describes, which is why it is written down rather than silently overwritten.
...
| EX-53 | Open | **The group H rules have a nominal owning record, not a real one.** All five rows in `rules.register.json` name ADR 0012 as `owningAdr`, because it owns the support-claim and ownership surface those rules protect. ADR 0012 is frozen and names none of them. Every other row in the register names a record that states its rule, so this is the one place where `owningAdr` asserts less than it looks like it asserts, and a reader should not read it as a binding the record itself carries. Closed by: the first amendment that lets a record state them, or a record of their own. |
