#!/usr/bin/env python3
"""E10 of VM-5-002's re-collection: long shapes through the command-line host, three lanes, two forms.

The first collection's measurement/measure-shapes.py, changed in three ways: the lanes directory is an
argument (--lanes-dir); the shapes are read from the first collection's measurement/shapes, which are
the files that run used; and the log begins with identity lines for each lane (identity.py, with the
digests of the lane's host executable, host assembly and runtime assembly), written before the first
repetition. Everything else - the interleaving, the warm-ups, the medians and the A/A spread - is the
first collection's.

Lanes are snapshotted binary directories, not trees: `base` is the base build's CLI output directory
(f127d92 with the E9 measurement patch), `credit` is the remedy head's (D:/broiler-arms/credit-measure,
with the same patch), and `aa` is a byte copy of `credit`, which makes the A/A lane an identical build
measured in the same interleaving.
...
python measure-shapes.py --lanes-dir <dir> --out <prefix>
"""
import argparse
import json
import os
...
SHAPES_DIR = HERE.parents[1] / "measurement" / "shapes"
...
ap.add_argument("--lanes-dir", required=True)
args = ap.parse_args()
LANES = pathlib.Path(args.lanes_dir)
...
for lane in args.lanes.split(","):
    ident = subprocess.run(
        [sys.executable, str(HERE / "identity.py"), "--build", "e10 lane " + lane, "--tree", str(LANES / lane),
         "--bin", str(LANES / lane / "Broiler.VM.Composition.JavaScript.Cli.exe"),
         "--bin", str(LANES / lane / "Broiler.VM.Composition.JavaScript.Cli.dll"),
         "--bin", str(LANES / lane / "Broiler.VM.Runtime.dll")],
        capture_output=True, text=True)
    for line in ident.stdout.splitlines():
        say(line)
...
for rep in range(-args.warmups, args.reps):
    for shape in shapes:
        for lane in lanes:
            for form in forms:
                exe = LANES / lane / "Broiler.VM.Composition.JavaScript.Cli.exe"
