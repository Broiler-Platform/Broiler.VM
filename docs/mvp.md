...
| MVP-7 | **The native output form is a WHOLE-ARTIFACT form under a restricted feature manifest.** A third manifest, `broiler.javascript.numeric`, admits a numeric subset of JavaScript and refuses every construct outside it at compile time, by name, with a diagnostic code and a source position; a program in it compiles in whole or not at all, so an artifact is native or it is bytecode and no handle carries two forms. There is no per-unit choice, no entry guard, no bailout, no fallback, no deoptimization and no promotion — the executor for a native artifact never reaches the interpreter, and a machine that cannot run the artifact's architecture refuses to instantiate it rather than running the other form | **A mixed-form artifact with per-unit compilation and an interpreter fallback**: compile the units a backend finds eligible, guard each emitted entry on the shape of its arguments, and let the interpreter carry everything else. **That is what a production engine would build**, it compiles far more of the language, and it is what this repository's published rules currently forbid — the JavaScript profile's amended non-goals pin *one form per handle, no promotion* and name a run-time form choice as the second execution arm they refuse, and the core's GC-rooting risk row makes a per-unit fallback the same thing under another name. It was the design the backend roadmap described until 2026-09-07 | **An amendment to the profile's non-goals, or a recorded refusal of one** — the rules that exclude the alternative are published records and not implementation limits, so the question is answerable only by changing one or by deciding not to. Until then the cost is paid in the open and is not small: **the compilable language is small and it is not JavaScript.** A native artifact runs numeric kernels; a program that touches a property, builds an array, holds a string or catches an error is refused before any artifact exists, where the alternative would have compiled part of it and run the rest. Widening the manifest is a different design with a rooting scheme in it and not a later increment of this one. **Ended 2026-09-15 by [JSC-215](../src/Broiler.VM.Profile.JavaScript/docs/roadmap.corrections.md#jsc-215); see [JSD-0025](../src/Broiler.VM.Profile.JavaScript/docs/decisions/0025-the-baseline-native-form-over-the-wide-manifest.md).** | The JavaScript profile's owner with the core's contract and security owners |
| MVP-8 | **The wide manifest's native form is subroutine-threaded.** Each instruction of an artifact compiled under `broiler.javascript.wide` with the native form is one call into a per-opcode instantiation of the JavaScript interpreter's own dispatch body; the control flow between instructions is emitted; every value stays in managed memory, and the emitted frame holds a handler table's address and an activation cookie and nothing else. That the wide manifest has a native form, and where its references are rooted, is decided by [JSD-0025](../src/Broiler.VM.Profile.JavaScript/docs/decisions/0025-the-baseline-native-form-over-the-wide-manifest.md); **the granularity at which it calls back — once per instruction rather than once per block — is weighed by no measurement, and is this row** | **Per-block steps**, which call once for a run of instructions rather than once for each and keep everything else; **or a value-carrying tier over an unmanaged value representation, with a rooting scheme** — the design the core's risk row makes a profile answer for, and the one the JavaScript profile's value-frame decision refuses today for every value-holding region | **A decision record adopting or refusing such a tier on a retained measurement of the baseline form's wall-clock cost.** No such measurement exists, and a figure taken in a working tree is not the retained measurement this row names. *(Corrected 2026-09-15, after collection: this cell read "No such measurement exists: the form's code is being written on this row's date, its evidence bundle is to be collected". The code was committed that day and the stage's bundle, [JSB-11-001](../src/Broiler.VM.Profile.JavaScript/docs/evidence/jsb-11-001/README.md), has been collected. It retains no measurement of the form's wall-clock cost taken as one and states none. The checks transcript it retains prints per-row times from a single run; the bundle does not read them, and they are not the retained measurement this row names. So this row's condition is unmet by it and the row stands)* | The JavaScript profile's owner with the core's security owner — one person |
| MVP-9 | **Fuel is admitted in blocks inside the core meter, and the change is read as implementing core contract version 1 rather than amending it** *(added 2026-09-16)*. A fuel charge that fits a block the meter has already checked at every budget level is admitted without the meter's lock and written to the counters before anything reads them. No block is taken under an aggregate parent, and the profile-facing surface is unchanged. ADR 0007's "Every link is checked, then every link is applied" and "every charge decrements the operation, instance, runtime and parent meters together" are read as what every observation point sees, not as a lock discipline. No record's text is revised to say so | **An allowance held by the profile**, sized by a new profile-facing member that reads remaining fuel — a breaking amendment, which the procedure of section 2.4 cannot mint. Or **a dated editorial revision of ADR 0007 describing deferred application**, which would add a permission to a contract-bearing record and so be an amendment itself. Or **one lock per charge, as before** | The core contract owner ruling whether the change implements version 1 or changes charging. If it changes charging, it changes behaviour behind an unchanged API, which ADR 0003 section 3 forbids: the product commits are reverted as a defect, and a candidate amendment is registered if the saving is still wanted | The core contract owner with the release and recertification owner — one person |
| MVP-10 | **ADR 0011's promise P1 gains `Broiler.VM.Ubc` by editorial revision, as the consequence of an extraction verdict the record's own clause says changes the core graph** *(added 2026-09-25; the universal bytecode concept's route UBC-R1, taken by filing [ADR 0013](adr/0013-the-universal-bytecode-extraction-record.md))*. P1 bounds a profile's Broiler.VM reference set to Abstractions and Binary; a profile that lowers to the universal bytecode references a third Broiler.VM-owned assembly, and this route reads that as the edge ADR 0013's verdict names rather than as a change to the contract. P1's text is edited, by a dated editorial revision, in the change where a profile first takes the edge, and not before *(taken 2026-09-25 at the programme's milestone UBC-2: the fixture family `Com.Example.Tally` is the first profile to take the edge, rule A13 is revised in the same change, and ADR 0011 carries the editorial revision; the ruling in the third column is still the one that would settle the reading)* | **An additive amendment minting core contract version 2**, which the procedure of section 2.4 cannot currently perform; the programme would then be `Blocked` naming the core as the holder | A ruling by the core contract owner on whether P1's member list is contract content or the recorded consequence of a gate the contract already contains. Either answer is an act, and silence is neither | The core contract owner — one person |
| MVP-11 | **The universal bytecode's primitive table is mechanism** *(added 2026-09-25; the concept's route UBC-R2, taken by filing [ADR 0013](adr/0013-the-universal-bytecode-extraction-record.md))*: a closed set of machine operations, stated in two's-complement and IEEE-754 terms, that an emitter may implement inline for a family row the family classifies as that primitive, on the condition that the inline implementation answers what the family's own handler answers, bit for bit, over a retained input corpus. It names no language, and a family whose language means something the primitive does not has the family's own dynamic row, not a primitive | **Typed arithmetic as rows of the common family, with one meaning the universal bytecode fixes**, which the concept's section 10 records and the request's placement of the 64-bit integer add with WebAssembly argues against | A dated verdict, in an extraction record or a revision of one, that examines the primitive table's entries and finds that they pass G3, or a refusal naming the entry that does not. ADR 0013 is not that verdict: its G3 finding covers its correspondence rows and the names of its proposed surface, and no row of its table describes the primitive table, which it says in its own words. The earliest such a verdict can be given is once the table exists as code, at the programme's milestone UBC-1 | The core architecture owner, with both language profiles' owners supplying their halves — one person |
| MVP-12 | **The standing refusal of "a shared value representation, frame layout, or opcode set" is reached by an extraction record's verdict for its opcode-set half, as an encoding whose language instructions keep the meanings their profiles give them** *(added 2026-09-25; taken by filing [ADR 0013](adr/0013-the-universal-bytecode-extraction-record.md))*. ADR 0011 lists that row among its standing refusals, "Refused permanently", in a table restated "so they are not re-litigated"; its failure clause reopens refused extraction records when "a language-free formulation is found", and that clause is written for records the gate produced, which the standing refusal is not. ADR 0013 accepts the universal bytecode anyway and says so, and the value-representation and frame-layout halves of the row stand | **The standing refusal holds as written**, and the universal bytecode is refused at the gate for its opcode-set half, which ends the programme with its record filed | A ruling by the core contract owner on whether a standing refusal of ADR 0011 may be reached by an extraction verdict, and if so by which procedure; or a dated editorial revision of ADR 0011 that says the second row's opcode-set half is no longer permanent, which would itself be read as an amendment of a contract-bearing record | The core contract owner, with the core architecture owner - one person |
| MVP-13 | **A `v` slot is an index into a family-owned managed plane, never a word emitted code holds** *(added 2026-09-25; the universal bytecode concept's route UBC-R3, taken at the programme's milestone UBC-2)*. The bytecode emitter keeps two planes per operation - a word plane of sixty-four-bit words and the family's own value plane, reached by index through `IUbcValuePlane` - and a `v` slot never occupies a word: the walk proves every slot's plane, so no instruction moves a value through the word plane, and a family's value type never enters the universal bytecode's assemblies. The loop reaches the value plane through that interface, one indirect call per plane operation, and the fixture composition's retained disassemblies show that call beside the folded family calls rather than leaving it to be found | **A NaN-boxed universal word with a handle table**, JSD-0035's value-word design made universal | A decision record adopting such a word on a retained measurement of the plane-by-index design's cost in a native form - the condition MVP-8 names for the JavaScript profile alone, now for every family. No such measurement exists, and milestone UBC-2 retains none | The JavaScript profile's owner with the core's security owner - one person |
| MVP-14 | **One dispatch loop generic over a family struct, instantiated per composed family** *(added 2026-09-25; the concept's route UBC-R7, taken at milestone UBC-2)*. `UbcInterpreter<TFamily>` constrains `TFamily` to a struct implementing `IUbcFamily` with static abstract members, and the loop calls the family's handler, landing, resumption, payload and codec members through the type argument. Bundle `ubc-2-001` retains the JIT's disassembly of the fixture instantiation's `Run`, `Start`, `Resume`, `Unwind` and `Suspend` - every method of the loop that calls the family - and the Native AOT image's of those of them that stand as symbols of their own; its fold check asserts that none holds a type test on the family struct or a dispatch through the family contract, and names the family members each calls directly. *(Corrected 2026-09-25, before the bundle was collected, when a review found the sentence claimed more than one retained listing could show. It read: "Bundle `ubc-2-001` retains the JIT's disassembly of the fixture instantiation's loop and the Native AOT image's disassembly of the same instantiation, and in both every family member is called directly, with no type test on the family struct".)* | **One loop per family, hand-written** | The route's own condition: a compiled loop that does not fold the family away - on the JIT lane or on a Native AOT image - reopens this row, and so does a family whose handler dispatch the fixed static signature cannot express. The value plane's interface call is not a family test, and it is MVP-13's subject rather than this row's | The core architecture owner - one person |
...
*(Corrected 2026-09-15.* MVP-3's route text described the emitted frame as a record of pointers "to
operand, local and constant storage, a fuel cell and a helper table", which named none of the operand
and local counts the frame declares, and the frame the numeric form declares has never carried a helper
table — its last field is the bytecode offset the emitted code had
reached when it stopped. From the same date a wide artifact's emitted frame is a different structure,
`JsBaselineFrame`, which holds a handler table's address and an activation cookie. **The row's
statement — the calling convention carries no managed reference at all — is true of both frames**, so
the row is not reopened, and its reopening observation is unchanged. And the limit the paragraph above
states, "the form is worth nothing on any program the numeric manifest does not admit", is now the
numeric form's limit rather than the native form's: a wide program has a native form of its own, the
one MVP-8 records, and what that form is worth is not a thing this record states either.)
...
**MVP-7 ended on 2026-09-15, on its own settling condition** *(added 2026-09-15)*. The profile's
non-goal was amended
([JSC-215](../src/Broiler.VM.Profile.JavaScript/docs/roadmap.corrections.md#jsc-215)), and
[JSD-0025](../src/Broiler.VM.Profile.JavaScript/docs/decisions/0025-the-baseline-native-form-over-the-wide-manifest.md)
took the decision beside it: the native form stays whole-artifact, and is admitted under the numeric and
the wide manifests, one form per handle and per instance. **The row stays in the table, marked ended,
rather than being deleted**, because a route that ends is still a route that was taken, and what was
built on it — the numeric form — is still standing and unchanged. The alternative it named, the
mixed-form artifact, is not what ended it: that remains what the amended rules forbid. What the amendment
admitted instead needed a row of its own, and **that row was filed while the work it records is being
written rather than after it landed**, which is the order this section asks for and the one MVP-7
itself did not get.

**MVP-8 is recorded because a decision can leave a route inside it undecided.** JSD-0025 decides that
the wide manifest has a native form, where that form's references are rooted and what it costs; it does
not decide that one call per instruction is the right granularity for ever, and it names a per-block
step and a value-carrying tier only to say this decision does not take them. A later form that took
either would be built on this row, and **what would settle it is a measurement that does not exist** —
so a record citing the baseline form's cost before a retained one exists is citing nothing.

**MVP-9 is recorded because reading ADR 0007's charging sentences as observable semantics is itself
a route** *(added 2026-09-16)*. The change rests on ADR 0003 section 6's "Not an amendment" -
implementing something version 1 already admits - and ADR 0003 is unapproved (section 2.1), so the
reading stands on a record nobody has approved as well as on a question nobody has ruled on. The
argument is in [bundle VM-5-002](evidence/vm-5-002/README.md) section 7.2. The fuel-exactness tests
...
**MVP-10 to MVP-12 are the universal bytecode programme's first routes, and MVP-10 is the one a
reader should be most uncomfortable with** *(added 2026-09-25)*. It is the one place the programme's
"no core amendment" is a reading of a record rather than a fact about code, and the reading is taken
by the person who would otherwise have to mint the version. The concept says so in its own section
3.3, and [ADR 0013](adr/0013-the-universal-bytecode-extraction-record.md) files the route rather than arguing it away. MVP-11 is the
route the concept's own section 10 names as the row to disagree with hardest: a primitive table is
arithmetic under another name, and what keeps it mechanism is that a family's handler is the meaning
and the primitive is checked against it, not the other way round. MVP-12 is the programme's own:
the concept argued that the standing refusal's reopening clause admitted its encoding, and a
review of the record found that the clause it cited is written for refused extraction records and
not for standing refusals, so the reopening is a route and says so. None of the three rows settles
anything, and the programme's later routes (the concept's UBC-R3 to UBC-R9) are filed here by the
milestone that takes each, not before.

**MVP-13 and MVP-14 are the two routes milestone UBC-2 takes** *(added 2026-09-25)*: the value plane
by index and the loop generic over the family. They are taken on a fixture family written to fit the
contract, which is evidence about the universal bytecode's own tests and not about a language, and
each names what would reopen it. MVP-13 is recorded with the one cost the milestone's own evidence
shows - the value plane is reached through an interface - so that a later reader weighing the
alternative starts from what the image does and not from what the concept expected.
