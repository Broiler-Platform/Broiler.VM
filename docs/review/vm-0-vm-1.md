...
| Check | Whether every figure quoted in a bundle, in the ledger and in the review file is the figure its log records: 7 projects Release with 0 warnings and 0 errors; 175 tests passing, 44 architecture and 131 behavioural; exactly 3 `.nupkg` and 3 `.snupkg`; a Native AOT binary of 1,279,488 bytes, recorded in the collecting machine's locale as `1.279.488`; a trimmed self-contained binary of 162,816 bytes; 5 checks passed and exit code 0 in each of the three modes; 4 negative controls |
| Falsified if | A document states a figure the log does not, in particular an AOT size other than 1,279,488 bytes or a suite total other than 175, or the SHA-256 list in `docs/evidence/vm-1/hashes.txt` does not recompute against the checkout the bundle claims to describe |
