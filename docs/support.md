...
of them.** *(Corrected 2026-09-15, after collection: that sentence is now true of the numeric form
only. The JavaScript profile's bundle
[JSB-11-001](../src/Broiler.VM.Profile.JavaScript/docs/evidence/jsb-11-001/README.md) retains a
collection over the wide-manifest form on one `win-x64` workstation, in the Windows convention only.
It claims no runtime identifier, it is not acceptance and nothing in it has been reviewed, so no cell
below claims more because of it, and no figure from it is copied here.)* *(Revised 2026-09-15, after the workflow runs: "in the Windows convention only" no longer holds for that bundle. It now also retains a machine-code workflow run of the conformance suite and selected lines of a pull-request lane job, both on hosted Linux runners, which executed the form's System V convention. That is not a support claim either: no runtime identifier is claimed, and `linux-x64` stays **Not claimed**.)* Every cell of the table below states what has been demonstrated **on retained evidence**,
...
working tree is not a collection. *(Corrected 2026-09-15, after collection: "nothing" stays true of
the numeric form and of every runtime-identifier claim. The wide-manifest form now has one retained
single-machine observation, in the Windows convention, in the JavaScript profile's bundle JSB-11-001,
and that observation claims no identifier.)* *(Revised 2026-09-15, after the workflow runs: that bundle
now also retains observations of its System V convention on hosted Linux runners, which claim no
identifier either.)* A reader who takes anything below as a claim about a runtime
...
| **x86-64 machine code** | **Yes, and it is emitted and executed.** The JavaScript profile's format carries an emitted-code section and a symbol section; its lowering carries encoders for both x86-64 calling conventions, named `x86-64-win64` and `x86-64-sysv`; its verifier answers for the payload **structurally**, **by a template-closure scan in every image**, and, where an image carries the lowering, by recompiling the artifact's own bytecode and comparing the emitted bytes; and one type in that profile maps a page and arms it readable-and-executable. *(Corrected 2026-09-08: this cell named two verification layers until this date, and the second of them — re-emission — reaches only an image that carries a compiler. An execution-only image carries none, so what stood between a malformed payload and an armed page there was the architecture value, which asks which machine bytes are for and not whether they are code: a corpus row of **four zero bytes** verified, was armed, was jumped into and killed the process on this date. A third layer now decodes an emitted payload against a closed table of every instruction template this build's backends emit and refuses one that is not closed under it, before anything is armed, in every image. **What that is worth is bounded in section 7 and is not a claim that the code is correct**, and the two rows of this table are unmoved by it: nothing here is retained evidence.)* **The language its numeric form compiles is `broiler.javascript.numeric`, and that is not JavaScript** — a program outside that manifest is refused at compile time by name *(corrected 2026-09-08: that last clause had one exception until this date and now has none. The front end admitted a **BigInt literal** and dropped its suffix, so `--numeric --native x86-64-win64` compiled `9007199254740993n` and emitted machine code returning `9007199254740992` — a plausible wrong integer produced by the armed native path rather than a refusal naming the construct. The parser now answers `2104:ConstructOutsideManifest` for the literal, which was checked by running that command line on this date. **The sentence is not being weakened, it is being dated**: a reader who took it on trust before today was told something this form did not do in one case, and a support table is the last document entitled to leave that unrecorded)* *(Corrected 2026-09-15: the bold sentence read "**The language it compiles is `broiler.javascript.numeric` and it is not JavaScript**" until this date, which made the x86-64 form and the numeric manifest one thing. From 2026-09-15 a second x86-64 form is being written in the same lowering, over `broiler.javascript.wide` — every unit emitted, and every instruction one call into the profile's own interpreter dispatch for that instruction — decided in the JavaScript profile's own decision record. **This table records no run of it and claims nothing for it**: its code is being written, no bundle retains any of it, and the rule at the head of this document — what has been demonstrated on retained evidence — gives it the answer it gives the numeric form, which is nothing. *(Corrected 2026-09-15, after collection: its code is committed, and the JavaScript profile's bundle JSB-11-001 retains a collection over it on one `win-x64` workstation in the Windows convention only. That is not a support claim and claims no runtime identifier, so what this table answers for the form is unchanged.)* *(Revised 2026-09-15, after the workflow runs: "in the Windows convention only" no longer holds for that bundle. It now also retains a machine-code workflow run of the conformance suite and selected lines of a pull-request lane job, both on hosted Linux runners, which executed the form's System V convention. That is not a support claim either: no runtime identifier is claimed, and `linux-x64` stays **Not claimed**.)* A reader must not read the existence of a wide-manifest form as a claim that JavaScript runs natively here, or as a statement of what either form is worth)* | **No runtime identifier is claimed, and none can be today.** The Windows convention was emitted and executed in a working tree on `win-x64`, answering what the interpreter answered for the same source; `win-x64` is **Not claimed** in section 3 for want of a retained collection, this row adds nothing to it, and a form cannot claim an identifier this component has not claimed for itself. **The System V convention has been emitted and executed nowhere**: on a Windows host it verifies and refuses to instantiate *(revised 2026-09-15: for the wide-manifest form, see the dated note at the end of this cell; that note says nothing of the numeric form)*. **No bundle retains any of this** *(corrected 2026-09-15, after collection: still true of the numeric form; for the wide-manifest form, its Windows convention emitted and executed on `win-x64` is now retained in the JavaScript profile's bundle JSB-11-001, as the note at the end of this cell says)*, and no cell here is a CI-lane result. The wide-manifest form of 2026-09-15 adds no runtime identifier and no observation to this cell *(corrected 2026-09-15, after collection: it still adds no runtime identifier. It adds one observation, retained rather than taken in a working tree: the JavaScript profile's bundle JSB-11-001 records the Windows convention of that form emitted and executed on `win-x64`, answering what the interpreter answered wherever it compares the two forms under the same allowance. Its System V convention has been executed nowhere, and `win-x64` stays **Not claimed**)* *(Revised 2026-09-15, after the workflow runs: for the wide-manifest form, "executed nowhere" no longer holds. The same bundle retains a machine-code workflow run of the conformance suite and selected lines of a pull-request lane job on hosted Linux runners, which executed its System V convention. It adds no runtime identifier: `linux-x64` and `win-x64` stay **Not claimed**)* | **Yes, and it is deterministic and by name.** An artifact whose architecture and convention the running process is not refuses to instantiate, as a contract violation naming an unsatisfied host assumption, rather than falling back to the bytecode beside it in the same artifact. And a composition that declines the native surface refuses the artifact **at verification**, with an invalid-artifact reason, before any instruction of it is reachable |
| **arm64 machine code** | **Yes as an encoder, and it is EMITTING-ONLY.** A backend named `arm64-aapcs64` emits into the same sections and its bytes are pinned against known-good encodings by golden rows in a checks lane. *(Corrected 2026-09-15: it emits for the numeric manifest only. The wide-manifest form the JavaScript profile's decision record of that date adds has no arm64 emitter; the arm64 backend is decided to refuse that manifest by name, and a wide artifact naming arm64 is refused at instantiation on every host. That code is being written and nothing about it is retained)* *(Corrected 2026-09-15, after collection: that code is committed, and the refusal is retained as one row of the slice compiler's checks in the JavaScript profile's bundle JSB-11-001. A retained refusal adds nothing to what this row claims)* **It has run nowhere**, and the reason is unchanged and is not a shortage of machines: the architecturally required instruction-cache maintenance sequence has no managed expression and no dependable library export | **None, and none is claimed. `emitting-only` is the word**, and it is in this cell rather than in a footnote because [release gate 11](roadmap.md#15-release-gates) and the native-execution milestone's exit gate require it here. Nothing has been published or run on any arm64 runtime identifier by this form, on any machine, at any time | **Yes.** An image asked to instantiate an arm64 artifact refuses by name, on every machine including an arm64 one, because no process this component runs in reports an architecture this build will arm for. The refusal is the same one the row above names and it is reachable from the command line |
...
- It does not say that a native artifact form is **supported**. Two now exist — section 3a says which,
  ...
  commit that emitted one.)* **Nor does it say so of the second x86-64 form**, over the JavaScript
  profile's wide manifest, whose code is being written from 2026-09-15: it adds no support claim, no
  runtime identifier and no statement of what it is worth *(added 2026-09-15)*. *(Corrected
  2026-09-15, after collection: "neither has a retained collection" and "being written" are now true
  of the numeric form only. The wide-manifest form's code is committed, and its Windows convention on
  one `win-x64` workstation is retained in the JavaScript profile's bundle JSB-11-001. It still has no
  claimed runtime identifier and no reviewed line, and this document copies no figure from that
  bundle.)* *(Revised 2026-09-15, after the workflow runs: that bundle now also retains its System V
  convention executed on hosted Linux runners, which adds no runtime identifier.)*
...
- It does not say that a **verified** native payload is correct code, and the layer added on
  ...
  layer that reaches the generator, and only an image carrying the compiler has it. **From 2026-09-15
  the scan is being given a second pair of tables** *(added 2026-09-15)*, for the wide manifest's
  form, whose one admitted call is an indirect call into a table of the interpreter's own handlers:
  closure under those tables is meant to show that every call lands in that table and not which
  handler belongs at which offset, which a handler checks when it runs — so a misplaced call is an
  internal defect rather than a wrong answer. **That is a statement of the design and not of a run**,
  and nothing in this repository retains one.
