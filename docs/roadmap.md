...
- a value representation, tagged value or frame ABI shared across languages, and any instruction
  set that gives a language's instructions meanings the language's own profile does not own. A
  shared *encoding* is not this non-goal: the universal bytecode - one instruction encoding, a
  common family and a primitive table of machine operations whose language-free meanings
  `Broiler.VM.Ubc` fixes, and a verifier walk, outside the core contract and referenced by nothing in
  the core - is admitted by [ADR 0013](adr/0013-the-universal-bytecode-extraction-record.md)'s extraction verdict, and every language
  instruction in it keeps the meaning its profile gives it
  *(revised 2026-09-25 through [ADR 0003](adr/0003-core-contract-v1-and-amendments.md) section 11,
  register row 19: this bullet read "one universal opcode set, tagged value, or frame ABI shared
  across languages". The refusal of a shared value and a shared frame ABI is unchanged; what moved
  is the opcode set, and only as an encoding whose meanings stay owned)*;
...
| Shared value representation, frame layout, or opcode set | **No.** These are the semantics the core exists not to own. *(Re-dated 2026-09-25 through ADR 0003 section 11, register row 20: the opcode-set half of this row has a new dated verdict, [ADR 0013](adr/0013-the-universal-bytecode-extraction-record.md), given in the next row as a shared encoding whose meanings stay owned; the value-representation and frame-layout halves stand exactly as written. The verdict above is not edited, because a reopening produces a new dated verdict rather than an edit of the old one.)* |
| The universal bytecode: one instruction encoding, container, common family, family-table schema, primitive table, verifier walk and dispatch loop, with every language instruction's meaning owned by the family that declares it (`Broiler.VM.Ubc`, `Broiler.VM.Emitter.Bytecode`) *(added 2026-09-25)* | **Yes, by extraction verdict of 2026-09-25 ([ADR 0013](adr/0013-the-universal-bytecode-extraction-record.md)).** G1: both product profiles implement framing, a code walk, a dispatch loop, charged activations, fault exits, per-instruction charges and immediate decoding in merged code; G2: the shared part is shown as a correspondence from that code; G3: the shared part names no language concept, checked by a banned-vocabulary scan the record retains; G4: no profile-to-profile edge. It is not part of the core contract and nothing in the core references it. **The native-form mechanism the concept places beside it (`Broiler.VM.Ubc.Native`) is not admitted: G1 is unsatisfied on the same date, because one product profile has a native form and the other has no emitted code at all, so the gate cannot be invoked for it, and ADR 0013 carries the dated note ADR 0011 prescribes for that state, which carries no verdict.** |
...
| **Output form** — which kind of artifact a lowering emits | yes | the same compiler. A backend is a **choice inside one lowering**, not a second lowering *(added 2026-09-07 for VM-7)*. Under the universal bytecode programme the choice may leave the lowering for an emitter family: a lowering emits universal bytecode only, and which form an artifact takes is chosen by the emitter a composition hands it to, which is still not a second lowering *(added 2026-09-25 through ADR 0003 section 11, register row 21, on [ADR 0013](adr/0013-the-universal-bytecode-extraction-record.md)'s verdict; nothing in the tree has moved yet)* |
...
| **Emitted code holds a managed reference the collector cannot see** | The core's promise that one runtime cannot corrupt another rests on the CLR's type and memory safety, which native frames leave. A profile whose value model carries a managed reference — the JavaScript profile's does, deliberately and on the record — cannot hand that reference to emitted code without a rooting scheme the collector understands. This is the profile's to solve and the core's to refuse: **the core admits a native form and does not thereby admit that any given value model may cross into it.** **Stop: a profile that cannot state where its emitted code's references are rooted has not earned the form, whatever its benchmarks say.** The JavaScript profile's statement for its form over its wide manifest, whose emitted code holds no reference, is [JSD-0025 section 4](../src/Broiler.VM.Profile.JavaScript/docs/decisions/0025-the-baseline-native-form-over-the-wide-manifest.md#4-where-references-are-rooted); a statement in a profile's record is that profile's answer and not evidence this component holds *(pointer added 2026-09-15)*. |
