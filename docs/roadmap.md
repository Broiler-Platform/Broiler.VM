...
- a value representation, tagged value or frame ABI shared across languages, and any instruction
  set whose instructions' meanings the core or a shared assembly owns. A shared *encoding* is not
  this non-goal: the universal bytecode - one instruction encoding, a common family whose every row
  means the same in any language, and a verifier walk, in `Broiler.VM.Ubc`, outside the core
  contract and referenced by nothing in the core - is admitted by [ADR 0013](adr/0013-the-universal-bytecode-extraction-record.md)'s extraction
  verdict, and every language instruction in it keeps the meaning its profile gives it
