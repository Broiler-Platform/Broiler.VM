...
| [PART] | **VM-5 — core overhead baselines** | **In progress** | [Evidence bundle VM-5-001](evidence/vm-5/README.md), collected 2026-08-29 on `linux-x64`. **A milestone whose subject was measurement changed the product assemblies twice, because a baseline asks a question no test asks: how much does this cost, and does the answer depend on anything it should not?** The guest-load mediator never reset its per-operation counters - `EnterScope` had an overload that omitted the operation's identity and every call site used it, so every step compared equal to the last and the reset never ran once. Fan-out, cumulative nested bytes and nested verifier work were therefore lifetime bounds on a mediator shared by every instance of one profile in one runtime: a probe asked how many mediated loads a runtime admits and printed **8**. Three existing fan-out tests passed throughout, because all three invoke exactly once. Separately, a runtime's capability depth lived in an `AsyncLocal<int>`, whose entry can never be released - returning the depth to zero stores a boxed zero, which is a present value - so one entry per runtime stayed on the thread for the life of the process, released by nothing and not by disposal: the same instantiate-and-invoke allocated 9,960 bytes early in a run and **1,188,872 bytes after seventy thousand runtimes**, and the benchmark took 528 seconds where it now takes 43. Nothing observable failed in either case. Fourteen projects build Release with **0 warnings**; **304 tests pass** (107 architecture, 197 behavioural); pack still produces exactly three packages; both compositions still publish and run in three modes with unchanged closures. **A benchmark host** - the one test-only project ADR 0001 revision 3 authorises, referencing no benchmarking package - produced **ten measurements on the JIT lane and on the Native AOT lane, every one with its A/A lane inside its effect**. `docs/baselines.md` is the register and rule **L1** binds it to the retained log in both directions. Seventeen negative controls each fail when injected and pass after revert, three of them new and each pinning one of this milestone's corrections. | **Open gate conditions.** (1) No review decision is recorded and no reviewer has read this work. (2) Every figure is an absolute time on **one four-processor machine** (EX-45, EX-88): the ratios travel, the absolute values do not, and a benchmark expires faster than a test result. (3) The gate's **envelope read and write** is answered for the operation-result envelope, not the persisted one, which is admitted by contract and implemented by no milestone (EX-96). (4) Only the register's measurement table is bound to the log; its recorded figures - startup, image sizes, the plateau and independence verdicts - are checked by nobody (EX-95). (5) The guest-load lane rebuilds a runtime inside its timed region because fan-out is a runtime-scope lifetime total whose profile maximum no host may raise (EX-97). (6) ADR 0001 owns rule group L nominally and names no rule of it (EX-94). (7) Package sizes are absent: packages are produced at VM-6. Next: review the two corrections against ADR 0008 and ADR 0004 - both are places where a bound the records state was not the bound the code enforced - and read the fan-out series, which is the one thing here that funds future optimisation work. **(8) Added 2026-09-16: the metering path changed for speed, and no registered baseline shows the saving.** The gate asks that "Optimization is funded only against one of these baselines". The only registered measurement on this path is `meter-per-instruction`, and the fixture executor it runs polls after every charge, so each block of fuel the changed meter admits covers one charge and is written back at the next poll: the row measures the write-back and the fresh block that moved into `Poll`, not the saving the change was made for, and its non-regression is all it could have shown. *(Corrected 2026-09-17: after the owner-directed remedy, `9e9377d`, the row measures a poll that counts the block without writing it back; it still polls after every charge and still cannot show the saving.)* Where the saving is measured - bundle VM-5-002's JavaScript shapes and its core probe - neither is a registered baseline. **So this change is not funded against a registered baseline in the gate's sense, and this row does not claim that it is.** The owner chose not to register a windowed-polling measurement and not to re-collect VM-6-001's benchmark log, and the gap is recorded as EX-112, beside EX-111 for the timing signal the change opens between concurrent operations of one runtime. The change - `e117162`, `a416c7a`, `738d9c7` and the resolution fallback `34dbd7a` - hits VM-5-001's recertification trigger "a change to the metering path, the bounded reader, or the guest-load mediator" again, in its first clause, the metering path; that trigger was first hit on 2026-08-31. *(Corrected 2026-09-17: this sentence quoted the trigger only as far as "a change to the metering path", as though that were the whole trigger; those commits change neither the bounded reader nor the guest-load mediator.)* VM-5-001's section 7 marks the funding clause `[MET]` on the ground that "No optimisation was performed", which no longer describes the tree, and `meter-per-instruction` in [the baseline register](baselines.md) describes code that no longer exists, as the register's note of this date says without changing a figure. Reading the change as an implementation of core contract version 1 rather than an amendment is a route taken without a decision, recorded as [MVP-9](mvp.md#5-routes-taken-without-a-decision). [Evidence bundle VM-5-002](evidence/vm-5-002/README.md) retains the change's evidence, reads its own predeclared rule as not met as a whole, and accepts nothing. *Added 2026-09-17:* the owner directed a remedy after seeing that result, and its two parts - `9e9377d`, a poll that counts a held block's work instead of settling and renewing it, and `965e6ad`, a capability boundary that puts back the context it was entered from - were accepted by the owner on 2026-09-17, after they were implemented. The bundle's re-collection on the remedy head reads the same rule again (its sections 5.11 and 7.4) and still reads it as not met as a whole: items 1, 3 and 4 hold there, and item 2 fails for the call shape in the native form. *Corrected 2026-09-17, after a review of the re-collection:* item 1 holds against a witness table fixed during development and committed after the runs, under which two witnesses no longer name the two-thread tests the first collection named for them, and those tests still pass under them (the bundle's section 5.14, item 9). *Added 2026-09-17, the owner's rulings:* the owner accepted both readings as the bundle records them - item 1 holds, with that qualification, and item 2 stays failing for the call shape in the native form, with no remedy, so nothing may describe the change as faster for that cell (the bundle's section 7.4). The rulings move nothing in this row and accept nothing. The remedy hits VM-5-001's recertification trigger "a change to the metering path, the bounded reader, or the guest-load mediator" again, in two of its clauses: the metering path, by `9e9377d`, and the guest-load mediator, by `965e6ad`, which changes `VmArtifactLoadMediator.Answer`. **What would close this clause:** registering a windowed-polling measurement in the benchmark register - a rule L1 register change, which re-collects VM-6-001's benchmark log - and collecting it before and after the change, or the performance owner recording the clause as unmet for this change. Re-collecting the register's bundle alone does not close it. No figure is copied here. |
...
*Added 2026-09-16: which core bundles the fuel pre-admission change reaches.* The change is
`e117162`, `a416c7a`, `738d9c7` and `34dbd7a`, over the base `f127d92`. In the product it adds
`VmFuelPreAdmissions.cs` and changes seven files of `Broiler.VM.Runtime`, one of them
(`VmVerification.cs`) in a comment only, and the generated `CODE-ASSURANCE.md`, `HUMAN_REVIEW.md`
and `assurance.manifest.json` were regenerated with it. **What follows is read off a hash comparison
run on this date, not taken from a plan**: for each core bundle and each file the change touches,
the digest of the file as the repository stores it at the head was compared with that bundle's
`hashes.txt` row, and so was its digest at `f127d92`. A **first hit** is a file the base still
matched and the head does not; **already stale** is a file the base already differed on, named with
the commit that first made it differ. No core bundle before VM-5-002 hashes the tests or fixture files the change touched.

| File | VM-1-003 | VM-2-001, VM-3-001, VM-4-001 | VM-5-001, VM-6-001 |
|---|---|---|---|
| `VmBudgetLevel.cs`, `VmExecutionScope.cs` | Already stale in bytes; a **first hit** in content (below) | **First hit** | **First hit** |
| `VmRuntime.cs` | Already stale (`9147ee5`, 2026-08-29) | Already stale (`c3ddfa7`, 2026-08-29; for VM-4-001 `e8a8cb7`, the same day) | **First hit** |
| `VmInstanceImplementation.cs`, `VmInstantiation.cs` | Already stale (`9147ee5`) | Already stale (`c3ddfa7`; for VM-4-001 `e8a8cb7`) | Already stale (`bef290d`, 2026-08-31) |
| `VmMeter.cs` | Already stale (`1bcb68e`, 2026-08-29) | Already stale (`bef290d`) | Already stale (`bef290d`) |
| `VmVerification.cs` | Already stale (`9147ee5`) | Already stale (`d5767c1`, 2026-08-31) | Already stale (`d5767c1`) |
| `CODE-ASSURANCE.md`, `assurance.manifest.json` | Already stale (`ffd9b4e` and `9147ee5`) | Already stale (`c3ddfa7`; for VM-4-001 `e8a8cb7`) | Already stale (`409e6ec` and `bef290d`) |
| `VmFuelPreAdmissions.cs` | New, and hashed in no bundle | | |

**VM-1-003's rows for the runtime files in the table are digests of working copies with CRLF line
endings**, taken over the content of `af071cd`, so none of those rows equals the bytes the
repository stores at any commit, and every one is already stale in bytes. Compared with line endings
normalised, `VmBudgetLevel.cs` and `VmExecutionScope.cs` were unchanged from that bundle until this
change, and the other runtime files were first changed by the commits the table names. Its rows for
the two generated files are of stored bytes, and the table's commits for them are exact.

What the comparison leaves, bundle by bundle:

- **VM-0-001 is not recertified.** Its expiry names any change to the component source. The one file
  it hashes that this change touches, `HUMAN_REVIEW.md`, has differed from its row since `350a7ba`
  (2026-08-27). It is re-collected when a release needs it.
- **VM-6-001's API baseline recertifies unchanged** under rule M1: `docs/api/public-api.txt` is
  byte-identical between `f127d92` and the head.
...
This paragraph re-collects nothing and moves no row.

*Added 2026-09-17: what the owner-directed remedy reaches.* The remedy is `9e9377d` and `965e6ad`, with
the test commits `4279862` and `542c715` and the remarks of `16e3d6d`, over `fc00248`. In the product it
changes `VmMeter.cs`, `VmRuntime.cs`, `VmCapabilityBinding.cs` and `VmArtifactLoadMediator.cs`, and a
remark in `VmFuelPreAdmissions.cs`; it changes `FuelChargeExactnessTests.cs` and
`ReviewRegressionTests.cs`; and the generated `CODE-ASSURANCE.md`, `HUMAN_REVIEW.md` and
`assurance.manifest.json` were regenerated with it. **The same hash comparison, run on this date** between
`fc00248` and `16e3d6d` against every core bundle's `hashes.txt`, **finds no first hit in any core bundle
before VM-5-002**: every row of theirs that names one of those files was already stale. For the files the
table above names, the commits it gives stand. Of the others, `VmCapabilityBinding.cs` has differed from
the rows of VM-1-003 to VM-6-001 since `1546342` (2026-08-31); `VmArtifactLoadMediator.cs` from
VM-1-003's since `cb0acba` (2026-08-29), from those of VM-2-001, VM-3-001 and VM-4-001 since `e8a8cb7`
(2026-08-29), and from those of VM-5-001 and VM-6-001 since `bef290d` (2026-08-31); and no core bundle
before VM-5-002 hashes either test file. **VM-5-002 was hit first** in every file it hashed that the
remedy changed, and its re-collection's commit regenerated its `hashes.txt` against the remedy head.
VM-0-001's `HUMAN_REVIEW.md` row stays stale since `350a7ba`, VM-6-001's API baseline still recertifies
unchanged - `docs/api/public-api.txt` is byte-identical from `f127d92` to the remedy head - and
