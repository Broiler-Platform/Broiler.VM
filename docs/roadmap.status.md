...
RID; the CI lane has since run on those two and collects no bundle, so it moves no claim either.
*Corrected 2026-09-01: this sentence said no CI lane had ever executed, which ADR 0001's revision
of the same date withdraws; it then said three RIDs, which the second revision of that date
narrows to two when the lane was brought back to ADR 0012's declared matrix.*
