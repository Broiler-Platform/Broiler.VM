...
| 19 | 1 | Non-goals | `one universal opcode set, tagged value, or frame ABI shared across languages;` | `a value representation, tagged value or frame ABI shared across languages, and any instruction set that gives a language's instructions meanings the language's own profile does not own. A shared *encoding* is not this non-goal: the universal bytecode - one instruction encoding, a common family and a primitive table of machine operations whose language-free meanings Broiler.VM.Ubc fixes, and a verifier walk, outside the core contract and referenced by nothing in the core - is admitted by ADR 0013's extraction verdict, and every language instruction in it keeps the meaning its profile gives it;` | 0013 | universal-bytecode-extraction-verdict, candidate A | Applied 2026-09-25 |
...
no public type of the core moves, and rule E2 asserts the set unchanged. The new
text of rows 19 and 22 was corrected on the same day, before the change left its
branch: as first applied, row 19 refused any instruction set whose meanings a
shared assembly owns, and row 22 kept verifier rules profile-owned and said the
